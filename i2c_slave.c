#include "i2c_slave.h"
#include "control.h"
#include "msg.h"
#include "periph/i2c.h"
#include <stddef.h>
#include <stdint.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <ztimer.h>
#define ENABLE_DEBUG 0
#include <debug.h>

#define REG_FIRMWARE_VERSION 0x01
#define REG_PROTOCOL_VERSION 0x02
#define REG_SENSOR_TYPE 0x03
#define REG_INIT_START 0x0A
#define REG_INIT_STATUS 0x0B
#define REG_MEAS_START 0x10
#define REG_MEAS_STATUS 0x11
#define REG_MEAS_TIME 0x12
#define REG_MEAS_DATA 0x20
#define REG_SENSOR_AMOUNT 0x30
#define REG_SENSOR_SELECTED 0x31
#define REG_MEAS_TYPE 0x32
#define REG_MEAS_SAMPLES 0x33
#define REG_SENSOR_DATA 0x38
#define REG_CONTROL_IO 0x40
#define REG_DIRECTION_IO 0x41
#define REG_ERROR_COUNT 0x50
#define REG_ERROR_STATUS 0x51

static const char fw_version[] = FW_VERSION;
static const uint8_t PROTO_VERSION = 0x00;
static const uint8_t MODULE_TYPE = 0xFF;
static const uint16_t MEASURE_TIME = 12000;
static const uint8_t SENSOR_COUNT = 0x01;
static uint8_t active_sensor = 0x01;
static uint8_t measurement_type = 0x00;
static uint16_t sample_count = 0x01;
static uint16_t sensor_value = 0x00;
static uint8_t error_value = 0x00;

uint16_t calculateCRC_CCITT(uint8_t *data, int length);

#define LEN_BYTES 1
#define CRC_BYTES 2
#define DATA_BYTES 52
#define REG_BYTES 1
// In case of READ, buffer is prefixed with a byte indicating the lenght of the
// data and suffixed with 2 CRC bytes In case of WRITE, buffer is prefixed with
// the register byte and suffixed with 2 CRC bytes
// #define BUFFER_SIZE LEN_BYTES + DATA_BYTES + CRC_BYTES
#define BUFFER_SIZE 255

uint8_t buffer[BUFFER_SIZE] = {0};

#define READONLY()                                                             \
    if (!read)                                                                 \
        return 0;
#define WRITABLE(size)                                                         \
    if (!read) {                                                               \
        buffer[0] = reg;                                                       \
        *data_ptr = &buffer[REG_BYTES];                                        \
        return size + CRC_BYTES;                                               \
    }

static msg_t msg = {0};
static uint8_t i2c_prepare(bool read, uint16_t reg, uint8_t **data_ptr,
                           void *arg) {
    (void)arg;
    DEBUG("p: %d, %02X\n", read, reg);
    uint8_t *data = buffer;
    uint8_t data_len = 0;
    switch (reg) {
    case REG_FIRMWARE_VERSION:
        READONLY();
        data_len = sizeof(fw_version);
        memcpy(data, (uint8_t *)fw_version, data_len);
        break;
    case REG_PROTOCOL_VERSION:
        READONLY();
        data_len = 1;
        data[0] = PROTO_VERSION;
        break;
    case REG_SENSOR_TYPE:
        READONLY();
        data_len = 1;
        data[0] = MODULE_TYPE;
        break;
    case REG_INIT_START:
        WRITABLE(1);
        data_len = 1;
        data[0] = do_initialize;
        break;
    case REG_INIT_STATUS:
        READONLY();
        data_len = 1;
        data[0] = initialize_status;
        break;
    case REG_MEAS_START:
        WRITABLE(1);
        data_len = 1;
        data[0] = do_measurement;
        break;
    case REG_MEAS_STATUS:
        READONLY();
        data_len = 1;
        data[0] = measurement_status;
        break;
    case REG_MEAS_TIME:
        READONLY();
        data_len = 2;
        data[0] = MEASURE_TIME & 0xFF;
        data[1] = (MEASURE_TIME >> 8) & 0xFF;
        break;
    case REG_MEAS_DATA:
        READONLY();
        struct __attribute__((packed)) {
            uint32_t conductivity_a;
            uint32_t conductivity_b;
            int16_t temperature_b;
            int16_t temperature_a;
        } wire_measurement = {
            .conductivity_a = measurement.conductivity_a,
            .conductivity_b = measurement.conductivity_b,
            .temperature_a = measurement.temperature_a,
            .temperature_b = measurement.temperature_b,
        };
        data_len = sizeof(wire_measurement) + 1;
        data[0] = sizeof(wire_measurement);
        memcpy(data + 1, (uint8_t *)&wire_measurement,
               sizeof(wire_measurement));
        measurement_status = STATUS_NOT_READY;
        break;
    case REG_SENSOR_AMOUNT:
        READONLY();
        data_len = 1;
        data[0] = SENSOR_COUNT;
        break;
    case REG_SENSOR_SELECTED:
        WRITABLE(1);
        data_len = 1;
        data[0] = active_sensor;
        break;
    case REG_MEAS_TYPE:
        WRITABLE(1);
        data_len = 1;
        data[0] = measurement_type;
        break;
    case REG_MEAS_SAMPLES:
        WRITABLE(2);
        data_len = 2;
        data[0] = sample_count & 0xFF;
        data[1] = (sample_count >> 8) & 0xFF;
        break;
    case REG_SENSOR_DATA:
        READONLY();
        data_len = 2;
        data[0] = sensor_value & 0xFF;
        data[1] = (sensor_value >> 8) & 0xFF;
        break;
    case REG_ERROR_STATUS:
        READONLY();
        data_len = 1;
        data[0] = error_value;
        break;
    default:
        return 0;
    }

    // any writes will have returned by now, due to the READONLY and WRITABLE
    // macros finish read by adding length and CRC bytes
    uint16_t crc = calculateCRC_CCITT(data, data_len);
    buffer[data_len] = (crc >> 8) & 0xFF;
    buffer[data_len + 1] = crc & 0xFF;

    *data_ptr = buffer;
    return LEN_BYTES + data_len + CRC_BYTES;
}

static void i2c_finish(bool read, uint16_t reg, size_t len, void *arg) {
    (void)arg;
    DEBUG("f: %d, %02X, %d\n", read, reg, len);
    if (read)
        return; // nothing to do
    if (len < 3)
        return; // Should atleast have 1 data byte + 2 CRC bytes

    // Skips the "reg" byte, which is used for CRC calc
    // I2C master writes to buffer[1...]
    uint8_t *data = &buffer[REG_BYTES];

    // Note: len does NOT include REG_BYTE at buffer[0]
    // Validate Checksum
    uint16_t crc = (data[len - 2] << 8) | data[len - 1];
    if (crc != calculateCRC_CCITT(buffer, len + REG_BYTES - CRC_BYTES)) {
        return;
    }

    // Execute command
    switch (reg) {
    case REG_INIT_START:
        msg.type = TASK_SENSOR_INIT;
        msg_send(&msg, main_thread_pid);
        break;
    case REG_MEAS_START:
        msg.type = TASK_MEASUREMENT;
        msg_send(&msg, main_thread_pid);
        break;
    case REG_SENSOR_SELECTED:
        active_sensor = data[0];
        break;
    case REG_MEAS_TYPE:
        measurement_type = data[0];
        break;
    case REG_MEAS_SAMPLES:
        sample_count = (data[0] << 1) | data[1];
        break;
    }
}

i2c_slave_fsm_t i2c_slave;
int i2c_slave_init(uint8_t addr) {
    //i2c_config[0].dev->CR1 &= ~(I2C_CR1_PE);
    /* configure slave addr (7 bits) */
    i2c_config[0].dev->OAR1 &= ~(I2C_OAR1_OA1EN |I2C_OAR1_OA1);
    i2c_config[0].dev->OAR1 |= I2C_OAR1_OA1EN | ((addr & 0x7F) << 1);
    i2c_slave_reg(&i2c_slave, i2c_prepare, i2c_finish, 0, NULL);
    //i2c_config[0].dev->CR1 |= I2C_CR1_PE;
    return 0;
}

static const uint16_t CRC16Table[] = {
    0x0000, 0xC0C1, 0xC181, 0x0140, 0xC301, 0x03C0, 0x0280, 0xC241, 0xC601,
    0x06C0, 0x0780, 0xC741, 0x0500, 0xC5C1, 0xC481, 0x0440, 0xCC01, 0x0CC0,
    0x0D80, 0xCD41, 0x0F00, 0xCFC1, 0xCE81, 0x0E40, 0x0A00, 0xCAC1, 0xCB81,
    0x0B40, 0xC901, 0x09C0, 0x0880, 0xC841, 0xD801, 0x18C0, 0x1980, 0xD941,
    0x1B00, 0xDBC1, 0xDA81, 0x1A40, 0x1E00, 0xDEC1, 0xDF81, 0x1F40, 0xDD01,
    0x1DC0, 0x1C80, 0xDC41, 0x1400, 0xD4C1, 0xD581, 0x1540, 0xD701, 0x17C0,
    0x1680, 0xD641, 0xD201, 0x12C0, 0x1380, 0xD341, 0x1100, 0xD1C1, 0xD081,
    0x1040, 0xF001, 0x30C0, 0x3180, 0xF141, 0x3300, 0xF3C1, 0xF281, 0x3240,
    0x3600, 0xF6C1, 0xF781, 0x3740, 0xF501, 0x35C0, 0x3480, 0xF441, 0x3C00,
    0xFCC1, 0xFD81, 0x3D40, 0xFF01, 0x3FC0, 0x3E80, 0xFE41, 0xFA01, 0x3AC0,
    0x3B80, 0xFB41, 0x3900, 0xF9C1, 0xF881, 0x3840, 0x2800, 0xE8C1, 0xE981,
    0x2940, 0xEB01, 0x2BC0, 0x2A80, 0xEA41, 0xEE01, 0x2EC0, 0x2F80, 0xEF41,
    0x2D00, 0xEDC1, 0xEC81, 0x2C40, 0xE401, 0x24C0, 0x2580, 0xE541, 0x2700,
    0xE7C1, 0xE681, 0x2640, 0x2200, 0xE2C1, 0xE381, 0x2340, 0xE101, 0x21C0,
    0x2080, 0xE041, 0xA001, 0x60C0, 0x6180, 0xA141, 0x6300, 0xA3C1, 0xA281,
    0x6240, 0x6600, 0xA6C1, 0xA781, 0x6740, 0xA501, 0x65C0, 0x6480, 0xA441,
    0x6C00, 0xACC1, 0xAD81, 0x6D40, 0xAF01, 0x6FC0, 0x6E80, 0xAE41, 0xAA01,
    0x6AC0, 0x6B80, 0xAB41, 0x6900, 0xA9C1, 0xA881, 0x6840, 0x7800, 0xB8C1,
    0xB981, 0x7940, 0xBB01, 0x7BC0, 0x7A80, 0xBA41, 0xBE01, 0x7EC0, 0x7F80,
    0xBF41, 0x7D00, 0xBDC1, 0xBC81, 0x7C40, 0xB401, 0x74C0, 0x7580, 0xB541,
    0x7700, 0xB7C1, 0xB681, 0x7640, 0x7200, 0xB2C1, 0xB381, 0x7340, 0xB101,
    0x71C0, 0x7080, 0xB041, 0x5000, 0x90C1, 0x9181, 0x5140, 0x9301, 0x53C0,
    0x5280, 0x9241, 0x9601, 0x56C0, 0x5780, 0x9741, 0x5500, 0x95C1, 0x9481,
    0x5440, 0x9C01, 0x5CC0, 0x5D80, 0x9D41, 0x5F00, 0x9FC1, 0x9E81, 0x5E40,
    0x5A00, 0x9AC1, 0x9B81, 0x5B40, 0x9901, 0x59C0, 0x5880, 0x9841, 0x8801,
    0x48C0, 0x4980, 0x8941, 0x4B00, 0x8BC1, 0x8A81, 0x4A40, 0x4E00, 0x8EC1,
    0x8F81, 0x4F40, 0x8D01, 0x4DC0, 0x4C80, 0x8C41, 0x4400, 0x84C1, 0x8581,
    0x4540, 0x8701, 0x47C0, 0x4680, 0x8641, 0x8201, 0x42C0, 0x4380, 0x8341,
    0x4100, 0x81C1, 0x8081, 0x4040};

/**
 * @brief Calculate the CRC CCITT
 * @param data The message to calculate the CRC from
 * @param length The lenght of the message
 * @return The 16 bits CRC
 */
#define CRC16INITVALUE 0xFFFF // Initial CRC value
uint16_t calculateCRC_CCITT(uint8_t *data, int length) {
    uint16_t crc = CRC16INITVALUE;
    while (length--)
        crc = (crc >> 8) ^ CRC16Table[*data++ ^ (crc & 0xFF)];
    uint16_t temp = ((crc << 8) & 0xFF00) | (crc >> 8); // swap
    return temp;
}
